...
int32_t read_file(int32_t fd, void* buf, int32_t nbytes)
{
    ...
    uint32_t offset;
    int32_t pid1;
    pid1 = terminal[curr_terminal].prog_array[terminal[curr_terminal].terminal_prog_count-1];
    inode = PCB_array[NUM_PROCESS-1-pid1].thread_info.file_array[fd].inode;
    offset = PCB_array[NUM_PROCESS-1-pid1].thread_info.file_array[fd].file_pos;
    /* Call read_data to read the data into the buffer.*/
    bytes_read = read_data(inode, offset, buf, nbytes);
    if (bytes_read == -1)
        return -1;
    /* Update the file position.*/
    PCB_array[NUM_PROCESS-1-pid1].thread_info.file_array[fd].file_pos += bytes_read;
    return bytes_read;
}
...
int32_t read_dir(int32_t fd, void* buf, int32_t nbytes)
{
    ...
    PCB_array[NUM_PROCESS-1-pid1].thread_info.file_array[fd].file_pos ++;
